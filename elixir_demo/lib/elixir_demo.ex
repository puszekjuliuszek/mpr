defmodule ElixirMPI.MPI do
  ...
  """
  def barrier(rank, size, world) do
    ...
    if coordinator == nil do
      ...
      send(coordinator, {:barrier_reached, rank, self()})
  ...
  defp process_loop(rank) do
    receive do
      {:mpi_message, from_pid, tag, data} ->
        IO.puts("Process #{rank}: Received message from #{inspect(from_pid)} with tag #{tag}")
        # Handle the message (in this case just log it)
        process_loop(rank)

      {:barrier_complete} ->
        IO.puts("Process #{rank}: Received barrier completion")
        process_loop(rank)

      {:EXIT, pid, reason} ->
        IO.puts("Process #{rank}: Linked process #{inspect(pid)} exited: #{inspect(reason)}")
        process_loop(rank)

      other ->
        IO.puts("Process #{rank}: Received unknown message: #{inspect(other)}")
        process_loop(rank)
    end
  end
end
...
defmodule ElixirMPI do
  ...
  def main(rank, size, world) do
    ...
    ElixirMPI.MPI.barrier(rank, size, world)
